...
function randFloat(min: number, max: number, dec = 4) {
...
export async function GET() {
  ...
  const agents = [
    ...
    { id: 'AGT-0089', name: 'Euro Trade Node', org: 'EU Markets', status: rand(0,3) > 0 ? 'ONLINE' : 'IDLE', vol_24h: rand(20000, 90000), trades: rand(80, 350), accuracy: randFloat(55, 80, 1) },
    ...
    { id: 'AGT-0301', name: 'Energy Markets Bot', org: 'EnergyCorp', status: rand(0,2) > 0 ? 'ONLINE' : 'IDLE', vol_24h: rand(15000, 70000), trades: rand(50, 250), accuracy: randFloat(52, 78, 1) },
  ]
  ...
  const signals = Array.from({ length: 12 }, (_, i) => ({
    ...
    source: agents[i % agents.length].name,
  }))
